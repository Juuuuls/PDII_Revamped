...
# ✅ Manual UTV1 offset input
# ✅ Manual UTV2 offset input
#
# Updated:
# ✅ Popup buttons removed for 7" touch usability
# ✅ Popup auto-closes after a short delay
# ✅ Popup can also be dismissed by tapping/clicking anywhere on it
#
# NOTE: UI changes only. No backend logic/functions altered.
# NOTE: dgpo5.py must support:
#       --utv1-offset
#       --utv2-offset
...
# ---------------- Paths / constants ----------------
SCRIPT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dgpo6.py")

ANGLE_STEP = 1.8
MAX_ROWS = 200
DEFAULT_CEILING_OFFSET = 100.0
DEFAULT_UTV1_OFFSET = 8.0
DEFAULT_UTV2_OFFSET = 8.0
...
def _predict_with_model(model, df, log_fn):
...
class App(ctk.CTk):
    ...
    # ---------- Popup helper (no buttons; auto-close for 7" touch) ----------
    def _popup(self, title: str, message: str, link_url: str | None = None, link_label: str = "Open Link"):
        def _show():
            ...
            grad.bind("<Button-1>", _close)
            box.bind("<Button-1>", _close)
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(adv, text="UTV1 Offset (cm)", text_color=TEXT_DIM, font=("Segoe UI", FONT_LABEL)).grid(
            row=5, column=0, sticky="w", padx=(12, 10), pady=(10, 6)
        )
        self.utv1_offset_var = ctk.StringVar(value=str(DEFAULT_UTV1_OFFSET))
        ctk.CTkEntry(
            adv,
            textvariable=self.utv1_offset_var,
            width=160,
            height=ENTRY_H,
            fg_color=FIELD,
            border_color=IG_PURPLE,
            corner_radius=12,
            font=("Segoe UI", 13),
        ).grid(row=5, column=1, sticky="w", padx=(0, 12), pady=(10, 6))

        ctk.CTkLabel(adv, text="UTV2 Offset (cm)", text_color=TEXT_DIM, font=("Segoe UI", FONT_LABEL)).grid(
            row=6, column=0, sticky="w", padx=(12, 10), pady=(10, 6)
        )
        self.utv2_offset_var = ctk.StringVar(value=str(DEFAULT_UTV2_OFFSET))
        ctk.CTkEntry(
            adv,
            textvariable=self.utv2_offset_var,
            width=160,
            height=ENTRY_H,
            fg_color=FIELD,
            border_color=IG_PURPLE,
            corner_radius=12,
            font=("Segoe UI", 13),
        ).grid(row=6, column=1, sticky="w", padx=(0, 12), pady=(10, 6))
        ...
        ctk.CTkCheckBox(
            ...
            checkbox_width=26,
            checkbox_height=26,
        ).grid(row=7, column=1, sticky="w", padx=(0, 12), pady=(0, 10))
        ...
        self._write("Ready — dgpo5 backend. Instagram-soft theme + gradient + popups + UTV1/UTV2 offset enabled.")
    ...
    def start(self):
        ...
        try:
            utv1_off = float(self.utv1_offset_var.get())
        except Exception:
            utv1_off = DEFAULT_UTV1_OFFSET

        try:
            utv2_off = float(self.utv2_offset_var.get())
        except Exception:
            utv2_off = DEFAULT_UTV2_OFFSET

        cmd.extend(["--utv1-offset", str(utv1_off), "--utv2-offset", str(utv2_off)])
