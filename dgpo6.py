...
(A) NEW 8 fields ✅ (recommended; includes raw + adjusted ceiling height)
    ...
    Example: 1,0.0,132,132,238,338,0.312,67.45
    - utv  = dist
...
Ultrasonic distance calibration control (Python-side):
- --utv1-offset X
  Applies to Sensor 1 distance only
- --utv2-offset Y
  Applies to Sensor 2 distance only

Offset behavior:
- final utv = parsed utv + sensor-specific offset
- positive offset increases displayed/output distance
- negative offset decreases displayed/output distance

IMPORTANT:
- This version DOES NOT automatically subtract 8 from raw_utv.
- In simulation, dist = rawDistance exactly.
"""
...
def main():
    ...
    parser.add_argument("--sketch", default=r"C:/Users/Ju/OneDrive/Documents/Project_Design_I_Files/pYcODE2/Fmain/Fmain.ino")
    ...
    parser.add_argument(
        "--apply-ceiling-offset",
        action="store_true",
        help="Apply ceiling-offset in Python (only if Arduino is not already applying it)."
    )

    # Ultrasonic distance calibration controls (Python-side)
    parser.add_argument(
        "--utv1-offset",
        type=float,
        default=0.0,
        help="cm offset to apply to Sensor 1 distance (utv). Can be negative."
    )
    parser.add_argument(
        "--utv2-offset",
        type=float,
        default=0.0,
        help="cm offset to apply to Sensor 2 distance (utv). Can be negative."
    )
    ...
    interval = args.interval if (args.interval and args.interval > 0) else (
        float(args.angle_step) / float(args.angle_speed)
    )
    ...
    print(f"[INFO] Sensor 1 UTV offset: {args.utv1_offset:+.3f} cm")
    print(f"[INFO] Sensor 2 UTV offset: {args.utv2_offset:+.3f} cm")
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        while written < rows_needed:
            ...
            try:
                ...
                # Apply sensor-specific ultrasonic offsets only if user set them
                if utv is not None:
                    if sensor == 1:
                        utv = float(utv) + float(args.utv1_offset)
                    elif sensor == 2:
                        utv = float(utv) + float(args.utv2_offset)
